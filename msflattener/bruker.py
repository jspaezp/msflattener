...
def get_timstof_data(path, min_peaks=15, progbar=True, safe=False):
    ...
    for start, end in my_iter:
        ...
        # TODO check if this can be a real range `range(start,end)`
        if safe:
...
def merge_ims_simple(df: pl.DataFrame, min_neighbors=3, mz_distance=0.01, progbar=True):
    ...
    my_iter = tqdm(
        zip(breaks[:-1], breaks[1:]), total=len(breaks) - 1, disable=not progbar, desc="Merging in 1d",
    )
    out_vals = {
        "mz_values": [],
        "corrected_intensity_values": [],
    ...
    logger.info(f"Finished simple ims merge, skipped {skipped} spectra for not having any peaks")
...
def centroid_ims(
    ...
    my_iter = tqdm(
        zip(breaks[:-1], breaks[1:]), total=len(breaks) - 1, disable=not progbar, desc="Merging in 2d",
    )
    out_vals = {
        "mz_values": [],
        "corrected_intensity_values": [],
    ...
    for start, end in my_iter:
        ...
        mzs = np.concatenate(chunk["mz_values"])
        imss = np.concatenate([[x]*len(y) for x, y in zip(chunk["mobility_values"], chunk["mz_values"])])
        intensities = np.concatenate(chunk["corrected_intensity_values"])
